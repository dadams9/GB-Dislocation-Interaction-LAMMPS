...
for step=89000:500:200000
    BaseName='zzFinedump.Ta3_550_N12_StressVolumePress1L_';
    ...
    slip_atomid   = 1478816;% %ID of the atom to get the location of that atom's information in the slip vector
...
% %      C = C';    
    ...
    G1_Ta3 = [5 1 5; -695 3875 -80; -19455 -3075 20070];

    G1_Ta3  = [G1_Ta3(1,1)/norm(G1_Ta3(1,:)) G1_Ta3(1,2)/norm(G1_Ta3(1,:)) G1_Ta3(1,3)/norm(G1_Ta3(1,:));   
               G1_Ta3(2,1)/norm(G1_Ta3(2,:)) G1_Ta3(2,2)/norm(G1_Ta3(2,:)) G1_Ta3(2,3)/norm(G1_Ta3(2,:));
               G1_Ta3(3,1)/norm(G1_Ta3(3,:)) G1_Ta3(3,2)/norm(G1_Ta3(3,:)) G1_Ta3(3,3)/norm(G1_Ta3(3,:))];
    
    %Find A which is the rotation matrix from
    %the lab frame into the crystal frame
    for i=1:3
       for j=1:3
           A(i,j) = dot(G1_Ta3(i,:),L(j,:));
       end
    end
        A = A';
...
k = 1; %Counter for the number of files read in
for step=155000:500:200000
    BaseName2='zzFinedump.Ta3_550_N12_StressVolumePress2R_';
    ...
    j=1;
    for i = 1:length(xR)
        if yR(i)>(cent_yR-20) && y(i)<(cent_yR+20) && sv4iR(i)>0 && zR(i)>10 && zR(i)<160
...
%     dR = [-2 1 -1];
    ...
    %Normalize the crystal frame matrix
    G2_Ta3 = [181 -23 125; -7 21 14 ;-2947 -3409 3640];
    G2_Ta3  = [G2_Ta3(1,1)/norm(G2_Ta3(1,:)) G2_Ta3(1,2)/norm(G2_Ta3(1,:)) G2_Ta3(1,3)/norm(G2_Ta3(1,:));   
               G2_Ta3(2,1)/norm(G2_Ta3(2,:)) G2_Ta3(2,2)/norm(G2_Ta3(2,:)) G2_Ta3(2,3)/norm(G2_Ta3(2,:));
               G2_Ta3(3,1)/norm(G2_Ta3(3,:)) G2_Ta3(3,2)/norm(G2_Ta3(3,:)) G2_Ta3(3,3)/norm(G2_Ta3(3,:))];
