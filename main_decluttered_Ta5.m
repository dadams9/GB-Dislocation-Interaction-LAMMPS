...
for step=89000:500:200000
    BaseName='zzFinedump.Ta5_550_N12_StressVolumePress1L_';
    ...
    slip_atomid   = 1685335;% %ID of the atom to get the location of that atom's information in the slip vector
...
% %      C = C';    
    ...
    G1_Ta5 = [28 27 16; -10 24 -23; -1005 484 942];

    G1_Ta5  = [G1_Ta5(1,1)/norm(G1_Ta5(1,:)) G1_Ta5(1,2)/norm(G1_Ta5(1,:)) G1_Ta5(1,3)/norm(G1_Ta5(1,:));   
               G1_Ta5(2,1)/norm(G1_Ta5(2,:)) G1_Ta5(2,2)/norm(G1_Ta5(2,:)) G1_Ta5(2,3)/norm(G1_Ta5(2,:));
               G1_Ta5(3,1)/norm(G1_Ta5(3,:)) G1_Ta5(3,2)/norm(G1_Ta5(3,:)) G1_Ta5(3,3)/norm(G1_Ta5(3,:))];
    
    %Find A which is the rotation matrix from
    %the lab frame into the crystal frame
    for i=1:3
       for j=1:3
           A(i,j) = dot(G1_Ta5(i,:),L(j,:));
       end
    end
        A = A';
...
k = 1; %Counter for the number of files read in
for step=87500:500:200000
    BaseName2='zzFinedump.Ta5_550_N12_StressVolumePress2R_';
    ...
    j=1;
    for i = 1:length(xR)
        if yR(i)>(cent_yR-20) && yR(i)<(cent_yR+20) && sv4iR(i)>0 && zR(i)>10 && zR(i)<160
    ...
    %dR = [mean(SD1R) mean(SD2R) mean(SD3R)];
            UdR(k,1) = dR(1);
            UdR(k,2) = dR(2);
            UdR(k,3) = dR(3);
    % dR = [-2 -1 -2];
    ...
    %Normalize the crystal frame matrix
    G2_Ta5 = [57 -42 12; 8 14 11 ;-630 -531 1134];
    G2_Ta5  = [G2_Ta5(1,1)/norm(G2_Ta5(1,:)) G2_Ta5(1,2)/norm(G2_Ta5(1,:)) G2_Ta5(1,3)/norm(G2_Ta5(1,:));   
               G2_Ta5(2,1)/norm(G2_Ta5(2,:)) G2_Ta5(2,2)/norm(G2_Ta5(2,:)) G2_Ta5(2,3)/norm(G2_Ta5(2,:));
               G2_Ta5(3,1)/norm(G2_Ta5(3,:)) G2_Ta5(3,2)/norm(G2_Ta5(3,:)) G2_Ta5(3,3)/norm(G2_Ta5(3,:))];
